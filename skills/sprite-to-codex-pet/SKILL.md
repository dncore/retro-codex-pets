...
1. **母表调色板优先探测法则**：
   - 若角色目录下同时存在全量母表（如 `*_full.png`）和单动作子图（如 `*_running.png`），第一时间检测色彩纯度（`len(np.unique(arr[non_bg, :3], axis=0))`）。
   - 若子图因导出工具的双线性插值带有数百至数千种混合过渡色，**严禁使用有损模糊子图，必须 100% 直接从全量母表中截取原生无损 15~16 色像素**！
2. **去底策略分流（闭合孔洞通透 vs 角色白部件保护）**：
   - **全局透明清零（适合 X 等暗白/非纯白装甲机体）**：角色白装甲为 `[240, 248, 248]` 等色阶，纯白 `(255, 255, 255)` 100% 专属于背景。直接全局 `is_bg = (arr[:, :, 0] == 255) & (arr[:, :, 1] == 255) & (arr[:, :, 2] == 255) -> transparent`，**彻底清空手臂与胸膛/双腿闭合环内的所有残留白色**！
   - **边界 BFS 泛洪保护（适合 Zero 等原生白部件机体）**：角色自带纯白护甲与高光，必须保留边界 BFS，严禁盲目全局删白或套用 `strip_halo`，防止角色白部件被误穿透掏空。
3. 确保所有 `Alpha == 0` 的区域其 RGB 绝对清零为 `(0, 0, 0, 0)`。

---

### 第二步：切片提取与黄金步态选择 (Frame Harvesting)
1. **确定缩放倍率**：复古 Sprite 高度通常在 30~48px 之间，在 208px 单元格中，最佳缩放比为 **4x** 或 **3x**（`Image.NEAREST` 整数最近邻倍率，大体型或宽动作优先 3x）。
2. **奔跑黄金循环 (Row 1, 2, 7)**：
...
3. **受挫倒地全链条 (Row 5 Failed)**：
   - **严禁混入受击闪电特效**：原画中浮动的 2 色电弧/高压闪光（Hit Shock Overlays）是引擎层覆盖特效，切勿将其当作角色身体帧拼凑！
   - **8 帧完整动力学链条**：`0: 受击硬直 -> 1: 后仰后退 -> 2: 击飞离地 -> 3: 滞空仰天 -> 4: 翻滚下落 -> 5: 背部撞地 -> 6: 倒地平躺 -> 7: 彻底瘫倒`。
   - 若倒地帧自带游戏内受创闪烁色板（Flash Palette），需 1:1 映射回标准常态色板。
...
2. **奔跑状态 (Row 1, 2, 7) —— 水平与垂直零抖动黄金法则**：
   ...
   - **先放大切片再镜像计算面部锚点 (Scale-First Mirroring)**：
     先对切片执行 `NEAREST` 整数缩放，再对放大后的图像进行镜像并计算 `get_face_anchor`，直接以放大后的像素网格对齐，彻底消除 1x 原始图整型除法取整导致的左跑 2.2px 抖动，将左右奔跑头部晃动全部稳稳压低至 `<= 0.7px`。
   - **水平面部锚点 (排除金翼/金甲高光污染)**：
     提取头部区域前 40% 高度（`arr[:int(bh * 0.40)]`），使用色相差 `r - g >= 25` 精准检测真实面部肉色，100% 滤除高光金甲/金翼（金黄 `r - g <= 18`）。将面部中线稳定锁定于 `target_face_x = 110.0 ~ 122.5px`（向左镜像为 `192.0 - target_face_x - 1.0`），全行动作头部水平漂移量严格 `<= 1.0px`。
   - **拖拽方向物理校验**：
